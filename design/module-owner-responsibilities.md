...
| `howl-term` | Terminal runtime orchestration across session, VT, and render; frame preparation; input publication; runtime wake/snapshot semantics; terminal C ABI | PTY variants, render backend internals, host platform UX | `src/terminal.zig`, `src/runtime/thread.zig`, `src/runtime/*.zig`, `src/render/*.zig`, `src/input/*.zig`, `src/wake/*.zig`, `src/config/*.zig`, `src/ffi.zig` |
...
- `runtime/lifecycle.zig`: construction, PTY startup, thread lifecycle, deinit.
