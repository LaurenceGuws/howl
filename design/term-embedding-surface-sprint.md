...
| Session I/O mechanics | `howl-session` | Outbound queueing/flush policy, transport waiting, bounded read pumping, resize propagation, and control signals are lower-owned. |
...
| Checkpoint 2: session-owned transport ownership | ready | Move owned transport construction/storage/attach/deinit out of `howl-term` if audit remains green. | `howl-term` no longer stores `howl_session.OwnedTransport`, calls `transport.pty()`, or deinitializes transport directly. |
| Checkpoint 3: lifecycle start/stop split | candidate | Decide whether session transport start/stop can become a single session-owned lifecycle operation while thread join/wake remains term orchestration. | Start/stop mechanics are split without session knowing VT/render/thread details. |
...
| --- | --- | --- | --- | --- |
| `runtime/lifecycle.zig` owns `howl_session.OwnedTransport` in `HowlTerm` | Constructs transport, stores it on term, attaches `transport.pty()` to session, deinitializes transport. | Ghostty `termio/Exec.zig` keeps PTY/subprocess ownership below `Surface`; Alacritty keeps PTY under terminal event loop/TTY layer. | `session-owned` | Checkpoint 2 candidate. Move owned transport storage/construction into `howl-session`; keep `HowlTerm.initPty` as embed constructor orchestration. |
| `runtime/lifecycle.zig` starts/stops session and term thread | Calls session start/stop while also handling thread flags, snapshot waiter stop, wake signal, and join. | Ghostty splits I/O backend lifecycle from surface/runtime thread handles; Alacritty event loop owns PTY I/O but UI owns app lifecycle. | mixed: `session-owned` for transport start/stop, `term-orchestration-only` for thread/wake/join | Do not move as one block. Split only after transport ownership lands. |
