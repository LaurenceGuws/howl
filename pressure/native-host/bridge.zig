...
pub export fn howl_native_control_destroy(raw: ?*anyopaque) void {
...
pub export fn howl_native_control_focus(raw: ?*anyopaque, focus_value: u8) i32 {
    const control = controlFromRaw(raw) orelse return 1;
...
pub export fn howl_native_control_signal(raw: ?*anyopaque, signal_value: u8) i32 {
    const control = controlFromRaw(raw) orelse return 1;
...
fn controlFromRaw(raw: ?*anyopaque) ?*Control {
    const value = raw orelse return null;
    return @ptrCast(@alignCast(value));
}

pub export fn howl_native_host_destroy(raw: ?*anyopaque) void {
...
fn observe(
    ...
    const measure_flags = measurementFlags(&rich);
    ...
    try writeHostHeader(&writer, begin, measure_flags);
...
fn writeHostHeader(writer: *Writer, begin: client.view.Begin, measure_flags: u32) !void {
...
const measure_begin = "HOWL_MEASURE_BEGIN";
const measure_end = "HOWL_MEASURE_END";

fn measurementFlags(snapshot: *const client.rich.Snapshot) u32 {
    var flags: u32 = 0;
    if (snapshotContainsAscii(snapshot, measure_begin)) flags |= 1 << 6;
    if (snapshotContainsAscii(snapshot, measure_end)) flags |= 1 << 7;
    return flags;
}

fn snapshotContainsAscii(snapshot: *const client.rich.Snapshot, needle: []const u8) bool {
    if (needle.len == 0) return true;
    for (snapshot.rows) |row| {
        var matched: usize = 0;
        for (row.cells) |cell| {
            for (cell.scalars) |scalar| {
                const byte: u8 = if (scalar <= 0x7f) @intCast(scalar) else 0;
...
fn decodeResidencies(host: *Host, bytes: []const u8) PressureError![]const canvas.Residency {
