...
const welcome = Buffer.from('48574c530802000000000008000000000000002a', 'hex');
const hello = '48574c530801000000000000';
...
function frame(kind, payload) {
  ...
  result.write('HWLS', 0, 'ascii'); result[4] = 8; result[5] = kind;
...
const interactionVector = Buffer.from('48574c53080d000000000014010203040506070800001ffd0403ff7f12340300', 'hex');
...
assert.equal(w.hw_send_text(1), 0);
const vectorCorpus = JSON.parse(await readFile('../howl-session/protocol/v8-vectors.json', 'utf8'));
