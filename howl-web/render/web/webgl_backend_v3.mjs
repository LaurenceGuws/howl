import {CommandV3 as C} from './frame_v3.mjs';
...
function clippedCommandSprite(command) {
  return clippedSpriteValues(
    command[C.x], command[C.y], command[C.width], command[C.height],
    command[C.clipX], command[C.clipY], command[C.clipWidth], command[C.clipHeight],
    command[C.sourceX], command[C.sourceY], command[C.sourceWidth], command[C.sourceHeight],
  );
}

function integralCommandScale(command) {
  const dw = command[C.width], dh = command[C.height];
  const sw = command[C.sourceWidth], sh = command[C.sourceHeight];
...
export function webglFrameEligible(frame) {
  for (const command of frame.commands) {
    if (command[C.kind] === 0) continue;
    if (command[C.kind] !== 1) return false;
    const visible = clippedCommandSprite(command);
    if (visible && !integralCommandScale(command)) return false;
  }
  return true;
}
...
export class WebGLTerminalBackend {
  ...
  syncResources(frame, framePixels, resourceKey) {
    ...
    const live = new Set();
    for (const command of frame.commands) if (command[C.kind] === 1) live.add(resourceKey(command[C.resource], command[C.generation]));
  ...
  append(destination, source, red, green, blue, alpha) {
    ...
    for (const command of frame.commands) {
      if (command[C.kind] === 0) {
        ...
        this.append(
          [command[C.x], command[C.y], command[C.width], command[C.height]],
          [0, 0, 0, 0],
          command[C.solidRed], command[C.solidGreen], command[C.solidBlue], command[C.solidAlpha],
        );
        continue;
      }
      const visible = clippedCommandSprite(command);
      ...
      this.append(
        visible.destination,
        visible.source,
        command[C.red], command[C.green], command[C.blue], command[C.alpha],
      );
    }
    this.flush(frame.surface);
