...
import {parseRendererFrameV4, selectRendererFrameV4} from '../web/frame_v4.mjs';
...
function parseFrames(bytes) {
  ...
  while (offset < bytes.length) {
    ...
    assert.equal(bytes[offset + 4], 10);
...
const frame = parseRendererFrameV4(w, decoder.decode(bytesAt(w.rv_frame_ptr(), w.rv_frame_len())));
assert.equal(frame.schema, 'howl.web-frame/v4');
assert.ok(frame.uploads.every(upload => upload.q.every(part => typeof part === 'string')));
assert.ok(frame.removals.every(removal => removal.every(part => typeof part === 'string')));
const rgba = [];
for (let i = 0; i < frame.commands.count; i += 1) if (frame.commands.kind(i) === 2) rgba.push(i);
assert.equal(rgba.length, 8);
const keys = new Map(misses.map(value => [value.imageId, qualified(value)]));
assert.deepEqual(rgba.map(i => [frame.commands.resource(i), frame.commands.generation(i)].map(String)), [
...
const stable = parseRendererFrameV4(w, decoder.decode(bytesAt(w.rv_frame_ptr(), w.rv_frame_len())));
let stableRgba = 0;
for (let i = 0; i < stable.commands.count; i += 1) if (stable.commands.kind(i) === 2) stableRgba += 1;
assert.equal(stableRgba, 8);
assert.equal(w.rv_ack(), 1);
