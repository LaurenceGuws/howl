//! Rasterizes the complete Unicode box-drawing range.
...
fn drawBoxHorizontalArm(pixels: []u8, width: u16, height: u16, style: BoxLineStyle, x0: u16, x1: u16, light_range: [2]u16, heavy_range: [2]u16, double_top: u16, double_bottom: u16, joins_up_double: bool, joins_down_double: bool, light: u16) void {
    if (x1 <= x0) return;
    switch (style) {
        .none => {},
...
fn centeredRange(size: u16, center: u16, thickness: u16) [2]u16 {
...
test "every line-spec box codepoint dispatches exactly once" {
    var codepoint: u32 = 0x2500;
    while (codepoint <= 0x257f) : (codepoint += 1) {
        const expected = codepoint <= 0x2503 or
            codepoint >= 0x250c and codepoint <= 0x254b or
            codepoint >= 0x2550 and codepoint <= 0x256c or
            codepoint >= 0x2574;
        try std.testing.expectEqual(expected, lineSpec(codepoint) != null);
    }
}
