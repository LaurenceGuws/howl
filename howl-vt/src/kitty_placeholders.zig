//! Pure Kitty Unicode-placeholder vocabulary and bounded diacritic mapping.
/// Kitty private-use scalar reserved for Unicode image placeholders.
...
/// Returns Kitty's one-based row/column value, or zero for a non-protocol diacritic.
pub fn oneBased(codepoint: u21) u16 {
    var lo: usize = 0;
    var hi: usize = diacritics.len;
    while (lo < hi) {
        const mid = lo + (hi - lo) / 2;
        const value = diacritics[mid];
        if (value < codepoint) lo = mid + 1 else hi = mid;
    }
    if (lo >= diacritics.len or diacritics[lo] != codepoint) return 0;
    return @intCast(lo + 1);
}

test "Kitty row-column diacritics retain canonical indices" {
    const std = @import("std");
    try std.testing.expectEqual(@as(u16, 1), oneBased(0x0305));
    try std.testing.expectEqual(@as(u16, 2), oneBased(0x030d));
    try std.testing.expectEqual(@as(u16, 3), oneBased(0x030e));
    try std.testing.expectEqual(@as(u16, 43), oneBased(0x059c));
    try std.testing.expectEqual(@as(u16, 0), oneBased(0x0300));
}
