...
pub fn fill(
    ...
    try std.testing.expect((try terminal.feed("\x1b F\x1b[5n")).stateChanged());
    ...
    for (0..eight_bit_queries) |_|
        try std.testing.expect((try terminal.feed("\x9b5n")).stateChanged());
    if (!eight_bit_after and eight_bit_queries != 0)
        try std.testing.expect((try terminal.feed("\x1b F")).stateChanged());
