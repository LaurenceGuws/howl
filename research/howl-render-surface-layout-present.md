...
- Required shape: keep SDL window/event/input harness and do not replace `SDL_GL_SwapWindow` in production behavior yet. Add a display-owned proof seam that binds `SDL_EGL_GetCurrentDisplay`, `SDL_EGL_GetWindowSurface`, and `SDL_EGL_GetProcAddress`, resolves `eglSwapBuffersWithDamageKHR`/`EXT`, validates main-thread/current-window/current-context preconditions, converts damage rect coordinates, and returns a decision enum: KHR possible, EXT possible, plain SDL swap fallback, or blocked because SDL Wayland swap semantics would be bypassed. This slice must not introduce a custom Wayland/backend seam.
- Tests/gates: fake-C tests proving KHR preferred over EXT, EXT fallback, plain swap fallback, zero/full damage fallback, rect coordinate conversion for EGL damage coordinates, token monotonicity unchanged, and main-thread/current-window/current-context assertions. Add one explicit test or source-backed assertion documenting whether the direct EGL call would bypass SDL Wayland `Wayland_GLES_SwapWindow`; if unresolved, status must be blocked, not ready for implementation. No live compositor CI requirement.
...
- Required shape: implement damaged present only if Slice 4 returns feasible. Keep SDL window/event/input harness. Use KHR if available, EXT if KHR unavailable, otherwise plain `SDL_GL_SwapWindow`. Call damaged present only at the existing present point after drawing and before token completion. Preserve main-thread assertions and synchronous completion semantics. If Slice 4 was blocked, this slice is not promotable.
...
- Host present tests: Slice 4 feasibility decision tests for KHR possible, EXT possible, plain SDL swap fallback, blocked SDL Wayland semantics, no damage/full damage fallback, EGL rect coordinate conversion, failed EGL call fallback/error behavior as chosen by slice, and present token completion unchanged. Slice 5 implementation tests are promotable only if Slice 4 is feasible.
...
- Direct `eglSwapBuffersWithDamageKHR/EXT` may bypass SDL’s Wayland `Wayland_GLES_SwapWindow` frame-callback wait/flush logic at `SDL_waylandopengles.c` lines 131-192. Slice 4 is proof-only and must block Slice 5 unless this is settled enough for KDE Wayland and Hyprland.
...
- Need Slice 4 proof that SDL-retained direct damaged swap does not break SDL Wayland frame callback pacing on KDE Wayland-only and Hyprland. Until then Slice 5 is blocked by design, not left to coder invention.
...
Accepted for execution through promoted slices only. The source-backed direction is executable without worker invention: exact names are fixed, allowed paths are explicit, VT dirty-column ABI work is assigned to Slice 3, and SDL/EGL damaged present is split into proof-first Slice 4 and implementation-only-if-feasible Slice 5. Alacritty governs the owner split and damaged-present shape; SDL remains the host harness unless proof shows direct damaged swap breaks required Wayland behavior; Kitty/GLFW and old Howl backend seams are evidence only.
