...
Researcher session id: `researcher-2026-06-17-recovery-slice-5-hyperlink-repair-08`.

Reviewer session id: pending Recovery Slice 5 hyperlink repair review.

Planning commit receipt: prior accepted root `b5f9eb5 Plan VT render state maturity sprint`; no-bridge recovery plan committed in root `87f37fc Recover VT render state sprint plan`; scope repair committed in root `e114b80 Repair VT recovery slice scope`; current Recovery Slice 5 hyperlink repair pending reviewer acceptance and receipt.
...
Recovery researcher session id: `researcher-2026-06-17-recovery-slice-5-hyperlink-repair-08`.

Recovery reviewer session id: pending Recovery Slice 5 hyperlink repair review.
...
- Required no-old-surface searches: `rg "HowlVtSurfaceResult|howl_vt_terminal_query_visible_meta|howl_vt_terminal_copy_surface|howl_vt_terminal_copy_surface_hyperlink|howl_vt_terminal_ack_surface|\bVtSurface\b|vtSurfaceFromResult|applyHyperlinkHover|applyHostCursorCadenceToVtSurface" howl-linux-host howl-render` must show no host/render product-code or compiled benchmark references. `rg "vt_surface" howl-render/src/benchmark_main.zig howl-render/src/cursor_presentation.zig howl-linux-host/src/terminal/links.zig` must show no old-surface benchmark/input/presentation/link references. `rg "howl_vt_terminal_copy_visible_hyperlink" howl-linux-host howl-vt` must show the new host link use plus VT declaration/route/export/tests. Test names may mention old symbols only to assert absence through header/ABI shape; compiled source may not keep old-surface owner names.
...
- Recovery Slice 5 hyperlink repair is pending reviewer acceptance before coder reseeding.
...
- Recovery Slice 5 now includes the omitted render cursor-presentation owner, compiled benchmark surface, host link owner, and new non-surface VT hyperlink ABI repair in exact allowed files, tests, verification, searches, and stop conditions.
- No coder is authorized. Host/render consumption remains blocked until reviewer accepts this Recovery Slice 5 hyperlink repair and orchestrator reseeds Recovery Slice 5.
