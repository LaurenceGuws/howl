...
Researcher session id: `researcher-2026-06-17-render-state-cell-abi-repair-correction-12`.

Reviewer session id: `reviewer-2026-06-17-recovery-slice-5-text-owner-repair-review-01`.

Planning commit receipt: prior accepted root `b5f9eb5 Plan VT render state maturity sprint`; no-bridge recovery plan committed in root `87f37fc Recover VT render state sprint plan`; scope repair committed in root `e114b80 Repair VT recovery slice scope`; hyperlink repair committed in root `b964014 Repair VT recovery hyperlink scope`; Recovery Slice 5 text-owner repair committed in root `4bd92d1 Repair VT recovery text owner scope`; current Recovery Slice 4B render-state cell ABI repair is pending reviewer gate and commit receipt.
...
Status: active recovery amendment blocked for predecessor render-state cell ABI repair before Recovery Slice 5.

Recovery researcher session id: `researcher-2026-06-17-render-state-cell-abi-repair-correction-12`.

Recovery reviewer session id: `reviewer-2026-06-17-recovery-slice-5-text-owner-repair-review-01`.
...
Accepted current blocker requiring predecessor repair:

- `howl-vt/src/ffi/render_state.zig` imports `ffi/surface.zig` and exposes render-state row cell data through `surface.FfiSurfaceCell`.
- Public render-state row cell reads therefore still depend on the old `HowlVtSurfaceCell` shape even though Recovery Slice 5 forbids `HowlVtSurfaceCell` in `howl-render` and forbids renderer-owned layout-compatible clones or substitutes.
- Recovery Slice 5 must not start until VT owns a dedicated public render-state cell ABI shape that can be consumed by `howl-render` without importing or recreating old surface cells.
...
- Allowed `howl-vt` files for hyperlink repair only: `howl-vt/include/howl_vt.h`, `howl-vt/src/terminal.zig`, `howl-vt/src/ffi/hyperlink.zig`, `howl-vt/src/ffi/main.zig`, `howl-vt/src/libhowl_vt.zig`, `howl-vt/test_ffi.zig`, `howl-vt/test/abi.zig`, `howl-vt/test_abi.zig`, `howl-vt/test_unit.zig`.
- Allowed `howl-linux-host` hyperlink file: `howl-linux-host/src/terminal/links.zig`.
- Allowed `howl-render` files: `howl-render/build.zig`, `howl-render/build.zig.zon`, `howl-render/include/howl_render.h`, `howl-render/src/c/prepare_request.zig`, `howl-render/src/c/prepare_request_test.zig`, `howl-render/src/c/test_support.zig`, `howl-render/src/libhowl_render.zig`, `howl-render/src/render_session.zig`, `howl-render/src/cursor_presentation.zig`, `howl-render/src/benchmark_main.zig`, `howl-render/src/vt_surface/surface.zig`, `howl-render/src/vt_surface/text_input.zig`, `howl-render/src/vt_surface/damage.zig`, `howl-render/src/vt_surface/cursor.zig`, `howl-render/src/vt_surface/theme.zig`, `howl-render/src/text/surface_preparer.zig`, `howl-render/src/text/direct_normal.zig`, `howl-render/src/text/shape/cluster.zig`, `howl-render/src/test_unit.zig`, `howl-render/src/test_abi.zig`.
...
- Required no-old-surface searches: `rg "HowlVtSurfaceResult|HowlVtSurfaceCell|howl_vt_terminal_query_visible_meta|howl_vt_terminal_copy_surface|howl_vt_terminal_copy_surface_hyperlink|howl_vt_terminal_ack_surface|\bVtSurface\b|VtSurfaceCandidate|sourceRenderableTextFromVtSurface|buildSparseVtSurfaceCellsWithDamageScratch|countVtSurfaceCodepoints|vtSurfaceFromResult|applyHyperlinkHover|applyHostCursorCadenceToVtSurface" howl-linux-host howl-render` must show no host/render product-code, render text-owner, or compiled benchmark references. `rg "vt_surface" howl-render/src/benchmark_main.zig howl-render/src/cursor_presentation.zig howl-render/src/text/direct_normal.zig howl-render/src/text/shape/cluster.zig howl-render/src/text/surface_preparer.zig howl-linux-host/src/terminal/links.zig` must show no old-surface benchmark/input/presentation/text/link references. `rg "howl_vt_terminal_copy_visible_hyperlink" howl-linux-host howl-vt` must show the new host link use plus VT declaration/route/export/tests. `rg "HowlVtRenderStateCell" howl-render howl-vt/include/howl_vt.h` must show renderer consumption of the VT-owned render-state cell ABI and the public VT declaration. Test names may mention old symbols only to assert absence through header/ABI shape; compiled source may not keep old-surface owner names.
...
- Stop conditions: any compatibility payload, bridge, shim, temporary `HowlVtSurfaceResult`, old `HowlVtSurfaceResult` inside a new host visible owner, renderer `VtSurface`, renderer-retained VT cells/dirty arrays/colors/cursor/selection/highlights, `cursor_presentation.zig` importing or mutating `VtSurface`, `benchmark_main.zig` keeping old `vt_surface` workloads/imports or old `HowlVtSurfaceCell` benchmark construction, `surface_preparer.zig`, `direct_normal.zig`, or `shape/cluster.zig` retaining `vt_surface`, `VtSurface`, `VtSurfaceCandidate`, `HowlVtSurfaceCell`, `sourceRenderableTextFromVtSurface`, `buildSparseVtSurfaceCellsWithDamageScratch`, or `countVtSurfaceCodepoints`, a separate text-owner cleanup requiring old surface compatibility, `links.zig` importing `vt_surface.zig`, `links.zig` calling old visible metadata, `links.zig` calling `howl_vt_terminal_copy_surface_hyperlink`, new hyperlink ABI implemented in `ffi/surface.zig`, new hyperlink ABI requiring `snapshot_seq` from host, host copied-cell hover mutation, test-only public substitute, hidden render work outside host/window pace control, link hover/open causing render-state update or render prepare from mouse movement, render prepare from outside `.prepare_needed`, render ABI still accepting `HowlVtSurfaceResult`, host/render product code still calling `howl_vt_terminal_copy_surface` or `howl_vt_terminal_ack_surface`, render check requiring a post-slice benchmark cleanup to pass, or renderer reading `howl-vt` Zig internals.
- Receipt fields: orchestrator session id `orch-2026-06-16-vt-render-state-planning-01`, researcher session id `researcher-2026-06-17-recovery-slice-5-text-owner-repair-09`, reviewer session id, coder session id, commit hashes for `howl-vt`, `howl-render`, `howl-linux-host`, and root pointer/receipt commits, all required verification results, no-old-surface search results, new hyperlink ABI search result, text-owner no-old-surface search result, and benchmark build result.
...
- Recovery Slice 5 text-owner repair is accepted by reviewer and pending commit receipt before coder reseeding.
...
- Recovery Slice 5 now includes the omitted render cursor-presentation owner, compiled benchmark surface, host link owner, new non-surface VT hyperlink ABI repair, and render text owners `direct_normal.zig` plus `shape/cluster.zig` in exact allowed files, tests, verification, searches, and stop conditions.
- No coder is authorized. Host/render consumption remains blocked until this Recovery Slice 5 text-owner repair has a root commit receipt and orchestrator reseeds Recovery Slice 5.
