...
Researcher session id: `researcher-2026-06-17-recovery-slice-5-scope-repair-07`.

Reviewer session id: pending Recovery Slice 5 scope repair review.

Planning commit receipt: prior accepted root `b5f9eb5 Plan VT render state maturity sprint`; no-bridge recovery plan committed in root `87f37fc Recover VT render state sprint plan`; current Recovery Slice 5 scope repair pending reviewer acceptance and receipt.
...
Recovery researcher session id: `researcher-2026-06-17-recovery-slice-5-scope-repair-07`.

Recovery reviewer session id: pending Recovery Slice 5 scope repair review.
...
- Required old-symbol handling: this slice must remove all host, renderer product-code, and render benchmark references to `HowlVtSurfaceResult`, `howl_vt_terminal_copy_surface`, `howl_vt_terminal_ack_surface`, renderer `VtSurface`, `vtSurfaceFromResult`, `applyHostCursorCadenceToVtSurface`, and `vt_surface` benchmark workload/input names. Old VT ABI declarations/functions may remain in `howl-vt` only for the later deletion slice, but they must have no host/render product, render benchmark, or build-check consumer.
...
- Required no-old-surface searches: `rg "HowlVtSurfaceResult|howl_vt_terminal_copy_surface|howl_vt_terminal_ack_surface|\bVtSurface\b|vtSurfaceFromResult|applyHyperlinkHover|applyHostCursorCadenceToVtSurface" howl-linux-host howl-render` must show no host/render product-code or compiled benchmark references. `rg "vt_surface" howl-render/src/benchmark_main.zig howl-render/src/cursor_presentation.zig` must show no old-surface benchmark/input/presentation references. Test names may mention old symbols only to assert absence through header/ABI shape; compiled source may not keep old-surface owner names.
...
- Stop conditions: any compatibility payload, bridge, shim, temporary `HowlVtSurfaceResult`, old `HowlVtSurfaceResult` inside a new host visible owner, renderer `VtSurface`, renderer-retained VT cells/dirty arrays/colors/cursor/selection/highlights, `cursor_presentation.zig` importing or mutating `VtSurface`, `benchmark_main.zig` keeping old `vt_surface` workloads/imports or old `HowlVtSurfaceCell` benchmark construction, host copied-cell hover mutation, test-only public substitute, hidden render work outside host/window pace control, render prepare from outside `.prepare_needed`, render ABI still accepting `HowlVtSurfaceResult`, host/render product code still calling `howl_vt_terminal_copy_surface` or `howl_vt_terminal_ack_surface`, render check requiring a post-slice benchmark cleanup to pass, or renderer reading `howl-vt` Zig internals.
- Receipt fields: orchestrator session id `orch-2026-06-16-vt-render-state-planning-01`, researcher session id `researcher-2026-06-17-recovery-slice-5-scope-repair-07`, reviewer session id, coder session id, commit hashes for `howl-render`, `howl-linux-host`, and root pointer/receipt commits, all required verification results, no-old-surface search results, and benchmark build result.
...
- Recovery Slice 5 scope repair is pending reviewer acceptance before coder reseeding.
...
- Recovery Slice 5 now includes the omitted render cursor-presentation owner and compiled benchmark surface in exact allowed files, tests, verification, searches, and stop conditions.
- No coder is authorized. Host/render consumption remains blocked until reviewer accepts this Recovery Slice 5 scope repair and orchestrator reseeds Recovery Slice 5.
