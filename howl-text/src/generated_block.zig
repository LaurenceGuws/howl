//! Rasterizes implemented block and braille codepoints.
...
fn rasterizeBlockElementAlpha(pixels: []u8, width: u16, height: u16, codepoint: u32) void {
    switch (codepoint) {
        ...
        0x2588 => geometry.fillRectAlpha(pixels, width, 0, 0, width, height, 255),
...
fn fillRows(pixels: []u8, width: u16, height: u16, start_eighth: u16, end_eighth: u16) void {
    ...
    while (eighth < end_eighth) : (eighth += 1) {
        const range = geometry.eighthPartitionRange(height, eighth);
        if (range.end > range.start) geometry.fillRectAlpha(pixels, width, 0, range.start, width, range.end - range.start, 255);
    }
}
...
fn fillCols(pixels: []u8, width: u16, height: u16, start_eighth: u16, end_eighth: u16) void {
    var eighth = start_eighth;
    while (eighth < end_eighth) : (eighth += 1) {
        const range = geometry.eighthPartitionRange(width, eighth);
        if (range.end > range.start) geometry.fillRectAlpha(pixels, width, range.start, 0, range.end - range.start, height, 255);
    }
}
...
fn drawBrailleDotAlpha(pixels: []u8, width: u16, height: u16, x0: u16, y0: u16, dot: u16) void {
    ...
    if (w == 1 and h == 1) {
        pixels[geometry.pixelOffset(width, x0, y0)] = 255;
        return;
    }
    ...
    while (y < h) : (y += 1) {
        ...
        while (x < w) : (x += 1) {
            ...
            if (alpha == 0) continue;
            const idx = geometry.pixelOffset(width, x0 + x, y0 + y);
...
test "every braille codepoint maps its exact eight-dot occupancy" {
    ...
    while (expected <= 0xff) : (expected += 1) {
        ...
        var actual: u8 = 0;
        const dot_at = [_]u8{ 0, 3, 1, 4, 2, 5, 6, 7 };
        for (dot_at, 0..) |dot, offset| {
            if (pixels[offset] != 0) actual |= @as(u8, 1) << @intCast(dot);
        }
        try std.testing.expectEqual(@as(u8, @intCast(expected)), actual);
    }
}
