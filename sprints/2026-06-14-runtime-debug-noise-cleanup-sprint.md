...
- Stop conditions: if any file outside the five allowed files needs content changes, stop and reseed exact file scope; if benchmark requirements cannot be satisfied without keeping timing fields in runtime owners, stop for reviewer/user decision; if `OwnedPreparedTextSurface` shape changes cascade into unrelated text owners beyond timing removal, stop and reseed exact owner scope; if assertions around mutex/prepared ownership or `PreparedHandle.create` ownership transfer are at risk, stop.
- Required receipt fields: planning seed receipt `e7a90db`, accepted planning receipt `1ccaddd`, Slice 2 correction acceptance receipt `e94864b`, exact decision for benchmark timing ownership, coder session id, reviewer verdict, verification commands/results, commit hash after orchestrator acceptance.
