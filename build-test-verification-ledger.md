...
| `test` | workspace root | Aggregate over package `test` steps | No root-owned install behavior | Runs package-local `test` surfaces only | Root orchestration aggregate | `test` | Concrete proof: the root maps directly to `howl-pty:test`, `howl-vt:test`, `howl-render:test`, and `howl-linux-host:test`; it proves only the union of currently exposed package test families. |
...
| `test` | `howl-vt` | Aggregate over `test:abi`, `test:unit`, and `test:regression` | No install wiring of its own | Runs the VT ABI, unit, and regression suites | Aggregate test step | `test` | Concrete proof: the public aggregate now covers all VT correctness-proof families and does not mix in fuzz or benchmark execution. |
...
| `test:regression` | `howl-vt` | `src/test/scrollback_regression.zig` | No install wiring of its own | Runs the `test-regression` Zig test binary | Regression test surface | `test:regression` | Concrete proof: root explicitly states that it replays deterministic VT scrollback churn and proves canonical logical content plus core invariants survive expensive resize and zoom scenarios. |
...
| Regression test root `test-regression` | `howl-vt` | `src/test/scrollback_regression.zig` | Installed only when `test:regression:build` is invoked | Run by `test:regression` | Regression | `test:regression` | Concrete proof: root now imports the shared helper under `scrollback_verifier`, not a fuzz-only label, and uses it only to replay explicit regression scenarios. |
...
- `howl-vt` `test:unit` remains broader than its label because the root imports FFI coverage and regression-flavored modules.
