...
| `render/howl-render-vulkan` | Vulkan backend executor | `howl_render_vulkan` exports only | same pattern as GL | may depend on `howl-render-core` only |
| `render/howl-render-software` | Software backend executor/reference | `howl_render_software` exports only | same pattern as GL | may depend on `howl-render-core` only |
...
| `howl-hosts/howl-android-host` | Android lifecycle/input/surface/process bridge | host-local app API only | split lifecycle/input/surface/process bridge modules | may depend on surface + selected renderer backend |
...
2. Linux POSIX PTY, Android container bridge, and future ConPTY must remain behind the same session transport contract.
...
3. Host repos call one render seam per frame; host code must not duplicate render planning policy.
