...
| `howl-term` | Embeddable terminal boundary (`howl-term`) for one terminal instance | `howl_term` exports only | lifecycle/input/frame/render-orchestration split | may depend on `howl-session` + `vt_core` + render-facing APIs |
...
| `howl-hosts/howl-sdl-host` | SDL host shell composition and platform loop | host-local app API only | split config/host-loop/platform entry/render handoff modules | may depend on one or more `howl-term` instances + selected renderer backend |
| `howl-hosts/howl-android-host` | Android host lifecycle/input/surface/process integration | host-local app API only | split host/input/platform-surface/process integration modules | may depend on one or more `howl-term` instances + selected renderer backend |
...
1. SDL and Android hosts are peer callers of the same `howl-term` API shape.
...
For MVP, the current repo name `howl-term` houses the owned boundary that is effectively
`howl-term`:
...
1. Any reverse dependency from `render-core` to `howl-term`.
